# Use Ubuntu 14.10 as base image
# FROM ubuntu:utopic
# Add Ubuntu mirrors
# RUN echo 'deb mirror://mirrors.ubuntu.com/mirrors.txt utopic main universe multiverse' > /etc/apt/sources.list
...
# PHP + Modules
RUN apt-get install -y php5 libapache2-mod-php5 php5-curl php5-mcrypt php5-gd php5-cli php5-sqlite php-apc php5-memcached memcached php5-mcrypt php5-svn php5-imagick php5-common php5-intl php5-pgsql
...
# MySQL
RUN bash -c "debconf-set-selections <<< 'mysql-server-5.5 mysql-server/root_password password $(cat /root/pw.txt)'" &&\
    bash -c "debconf-set-selections <<< 'mysql-server-5.5 mysql-server/root_password_again password $(cat /root/pw.txt)'"
RUN apt-get install -y mysql-server-5.5 mysql-client php5-mysql
...
# Entrypoint
ADD ./startup6.sh /root/run.sh
RUN chmod +x /root/run.sh
ENTRYPOINT ["/root/run.sh"]

# Ports SSL-Apache-MySQL
